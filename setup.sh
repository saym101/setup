...
clear
shopt -s extglob
# Функция-якорь для навигации
__TOP_() { :; }
# === 0. Проверка прав (Должна быть первой!) ===
if [[ $EUID -ne 0 ]]; then
...
DATE=$(date "+%Y-%m-%d")
standard_packages="fail2ban curl wget git sudo htop iotop ncdu mc zip unzip 7zip dnsutils net-tools iproute2 ca-certificates gnupg chrony openssh-server openssh-client lynx rsync"
chrony_servers="0.ru.pool.ntp.org 1.ru.pool.ntp.org 2.ru.pool.ntp.org 3.ru.pool.ntp.org"

# Читаем текущий порт из sshd_config в /etc/ssh при запуске
...
setup_hostname() {
    ...
    if confirm "${colors[y]}Хотите изменить имя хоста?${colors[x]}" "n"; then
        ...
        # Более надежная замена в /etc/hosts (учитывает табы и пробелы)
...
setup_software() {
    ...
    if confirm "${colors[y]}Установить набор программ?${colors[x]}" "n"; then
        ...
        while true; do
            read -e -i "$standard_packages" -p "${colors[y]}Список программ можно изменить(добавить или удалить) или оставить как есть:${colors[x]} " user_input
        ...
        # Используем apt-get для скриптовой стабильности
        apt-get update
        apt-get install -y $user_input || echo "${colors[r]}Ошибка при установке пакетов.${colors[x]}"
...
setup_chrony() {
    ...
    if ! command -v chronyc >/dev/null 2>&1 || ! [ -f /etc/chrony/chrony.conf ]; then
        ...
        if confirm "${colors[y]}Установить Chrony?${colors[x]}" "y"; then
            ...
            systemctl restart chrony && chronyc sources || echo "${colors[r]}Ошибка при перезапуске Chrony.${colors[x]}"
        fi
        return
    fi
    ...
    if confirm "${colors[y]}Хотите сменить NTP-серверы в настройках?${colors[x]}" "n"; then
        ...
        while true; do
			...
			read -e -i "$default_servers" -p "${colors[r]}Ваш выбор: ${colors[g]}" input_servers
...
# 6. Функция для настройки доступа root через SSH
setup_ssh_keys() {
...
# Проверяем, нет ли уже такого публичного ключа в файле
...
chmod 600 "$authorizedfile"
		[ -f /etc/ssh/sshd_config ] && cp /etc/ssh/sshd_config /etc/ssh/sshd_config.backup_$(date +%F_%H-%M-%S)
        # Безопасная настройка sshd_config
...
change_ssh_port() {
    ...
    local current_ssh_port=$(grep -E '^[[:space:]]*Port[[:space:]]+[0-9]+' "$sshconfigfile" | awk '{print $2}')
    ...
    # Если пусто — считаем как 'y' (случайный порт)
...
configure_ufw() {
    ...
    # 3. МЕНЮ НАСТРОЙКИ (Выполняется только если UFW установлен)
    while true; do
        clear
        ...
        echo "---------------------------------"
        read -p "Выберите действие: " ufw_choice
        ...
        case $ufw_choice in
            ...
            2)
                ...
                ;;
            3)
                read -p "Введите порт или название сервиса (напр. 80 или http): " p_allow
                ufw allow "$p_allow"
                ;;
            4)
                read -p "Введите порт для запрета: " p_deny
                ufw deny "$p_deny"
                ;;
            5)
                ufw status numbered
                read -p "Введите НОМЕР правила для удаления: " p_del
                ufw delete "$p_del"
                ;;
            6)
...
# === 0. ПОДГОТОВКА ПАПКИ ДЛЯ ДАННЫХ ===
local login_dir="${PWD}/login"
mkdir -p "$login_dir"
chmod 700 "$login_dir"

# === 0.1 ПОКАЗАТЬ СУЩЕСТВУЮЩИЕ ФАЙЛЫ ===
mapfile -t existing_files < <(ls -1 "$login_dir"/*_temp_*.txt 2>/dev/null | sort)
...
if [ $total_files -gt 0 ]; then
    ...
    while true; do
        ...
        if [ -z "$file_choice" ]; then
            ...
            if [ -f "$selected_file" ]; then
                ...
                # Вопрос о продолжении просмотра
    ...
    if confirm "Хотите добавить нового пользователя?" "n"; then
        ...
        # === 3. ВВОД ИМЕНИ НОВОГО ПОЛЬЗОВАТЕЛЯ ===
        clear
        ...
        # === 8. СОХРАНЕНИЕ ДАННЫХ В ПАПКУ login ===
        ...
        # === 9. ПРОВЕРКА НА СТАРЫЕ ФАЙЛЫ ЭТОГО ПОЛЬЗОВАТЕЛЯ ===
        local old_files=$(ls -1 "$login_dir"/${new_user}_temp_*.txt 2>/dev/null | grep -v "$pass_file")
        if [ -n "$old_files" ]; then
...
# 10. Функция для установки и настройки fail2ban
setup_fail2ban() {
    echo "${colors[g]}10] Установка и настройка fail2ban${colors[x]}"
    local_jail="/etc/fail2ban/jail.local"
    local_jail_d="/etc/fail2ban/jail.d"
    ...
    # === 2. ФУНКЦИЯ ПОЛУЧЕНИЯ ВСЕХ ЗАЩИЩЁННЫХ ПОРТОВ ===
    get_all_protected_ports() {
        ...
        if [ -d "$local_jail_d" ]; then
            for conf in "$local_jail_d"/*.conf; do
                [ -f "$conf" ] && ports_list+="$(grep -E "^port\s*=" "$conf" 2>/dev/null | cut -d'=' -f2 | tr -d ' ')"$'\n'
            done
        fi
        echo "$ports_list" | tr ',' '\n' | grep -E '^[0-9]+$' | sort -u
    }

    # === 3. ФУНКЦИЯ ПРОВЕРКИ КОНФЛИКТОВ ===
    check_port_conflicts() {
        local new_ports="$1"
        local existing_ports=$(get_all_protected_ports)
        local conflicts=""
    ...
    # === 4. ФУНКЦИЯ ПОИСКА ФАЙЛА С ПОРТОМ ===
    find_rule_by_port() {
        local search_port="$1"
        local found_file=""
        if [ -d "$local_jail_d" ]; then
            for conf in "$local_jail_d"/*.conf; do
                [ -f "$conf" ] || continue
                local ports=$(grep -E "^port\s*=" "$conf" 2>/dev/null | cut -d'=' -f2 | tr -d ' ')
        ...
        if [ -z "$found_file" ] && [ -f "$local_jail" ]; then
            local ports=$(grep -E "^port\s*=" "$local_jail" 2>/dev/null | cut -d'=' -f2 | tr -d ' ')
            if echo "$ports" | tr ',' '\n' | grep -qx "$search_port"; then
    ...
    # Базовый конфиг только если нет
    if [ ! -f "$local_jail" ]; then
        echo "[DEFAULT]" > "$local_jail"
        echo "bantime  = 10h" >> "$local_jail"
        echo "findtime  = 20m" >> "$local_jail"
        echo "maxretry = 5" >> "$local_jail"
        echo "ignoreip = 127.0.0.1/8 ::1" >> "$local_jail"
        echo "${colors[g]}Создан базовый конфиг: $local_jail${colors[x]}"
    fi

    # === 5. ОСНОВНОЙ ЦИКЛ МЕНЮ ===
    while true; do
        clear
        fail2ban_version="$(fail2ban-client version)"
        ...
        # Показ текущих правил с портами
        echo "${colors[c]}Текущие активные правила:${colors[x]}"
        local has_rules=false
        
        if [ -f "$local_jail" ]; then
            local jail_ports=$(grep -E "^port\s*=" "$local_jail" 2>/dev/null | cut -d'=' -f2 | tr -d ' ')
        ...
        if [ -d "$local_jail_d" ]; then
            for conf in "$local_jail_d"/*.conf; do
                ...
                local conf_ports=$(grep -E "^port\s*=" "$conf" 2>/dev/null | cut -d'=' -f2 | tr -d ' ')
        ...
        case $action_choice in
            1)
                # === ДОБАВЛЕНИЕ ПРАВИЛА С ПРОВЕРКОЙ КОНФЛИКТОВ ===
                echo ""
                ...
                case $port_choice in
                    ...
                    3)
                        jail_name="postfix"
                        jail_port="25,465,587"
                        jail_log="/var/log/mail.log"
                        ;;
                    4)
                ...
                # === ПРОВЕРКА КОНФЛИКТОВ ===
                if [ -n "$jail_port" ]; then
                    local conflicts=$(check_port_conflicts "$jail_port")
                    if [ -n "$conflicts" ]; then
                        echo ""
                        ...
                        for conflict_port in $conflicts; do
                            local conflict_file=$(find_rule_by_port "$conflict_port")
                        ...
                        case $conflict_choice in
                            1)
                                ...
                                ;;
                            2)
                                echo "${colors[r]}Добавляем с риском конфликта.${colors[x]}"
                                ;;
                            3)
...
clean_apt_cache() {
    echo "${colors[g]}10] Очистка системного кэша${colors[x]}"
...
# 11. Перезагрузка
reboot_system() {
    echo "${colors[g]}11] Завершение настройки${colors[x]}"
...
while true; do
    ...
    read -p "${colors[y]}Введите номер:${colors[x]} " choice
