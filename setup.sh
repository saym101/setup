...
################################################################################
# 6. Функция для настройки NTP сервиса
setup_ntp() {
...
################################################################################
# 8. Функция для изменения порта SSH и настройки UFW
...
echo
echo ${r}"Запускайте этот скрипт c правами root. Используя команду su -l "${x}
echo ${r}"Задав предварительно пароль для root коммандой 'sudo passwd root' "${x}
echo
    echo ${y}"Выберите номер нужного пункта:"${x}
    echo ${c}"1.${x}  ${g}Изменить hostname"${x}
    echo ${c}"2.${x}  ${g}Изменить локаль"${x}
    echo ${c}"3.${x}  ${g}Изменить часовой пояс"
    echo ${c}"4.${x}  ${g}Изменить репозитории и обновить систему"${x}
    echo ${c}"5.${x}  ${g}Установить ПО"${x}
    echo ${c}"6.${x}  ${g}Настроить NTP"${x}
    echo ${c}"7.${x}  ${g}Настроить SSH ключи"${x}
    echo ${c}"8.${x}  ${g}Изменить порт SSH и настроить UFW"${x}
    echo ${c}"9.${x}  ${g}Добавить пользователя"${x}
    echo ${c}"10.${x} ${g}Очистить apt кэш"${x}
    echo ${c}"11.${x} ${g}Перезагрузить систему"${x}
    echo ${c}"12.${x} ${g}Запустить выполнение всех пунктов по порядку"${x}
    echo ${c}"0. Выход"${x}

    read -p ${y}"Введите номер: "${x} choice
